...
mSDLWindow = SDL_CreateWindow(
     ...
     SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN | SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE 
    );
...
//SDL_GLContext glcontext = SDL_GL_CreateContext(mSDLWindow);
 //SDL_WindowData *data = (SDL_WindowData *) mSDLWindow->driverdata;
	

//mSDLWindow=SDL_CreateWindowFrom(native_window);*/		
  ...
  // opt["externalGLContext"] = Ogre::StringConverter::toString((int) 0 );
  //params.insert(std::make_pair("title", title));
    opt.insert(std::make_pair("FSAA", settings.fsaa));
    opt.insert(std::make_pair("vsync", settings.vsync ? "true" : "false"));
   // opt["externalGLContext"]    = Ogre::StringConverter::toString( (int)wmInfo.info.android.context );
    mWindow = Ogre::Root::getSingleton().createRenderWindow("OpenMW", settings.window_x, settings.window_y, true, &opt);
